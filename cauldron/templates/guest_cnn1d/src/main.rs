//! 1D CNN template (time_series -> score), integer-only
#![no_std]
#![no_main]
...
pub extern "C" fn rust_main() -> ! {
    unsafe {
        ...
        if INPUT_LEN < KERNEL_SIZE || STRIDE == 0 {
        ...
        let conv_out_elems = out_len * OUT_CHANNELS;
        let pooled_ptr = conv_buf + (conv_out_elems * 4) as u64;

        let base = WEIGHTS_DATA_OFFSET + WEIGHTS_OFFSET;
        let w1_base = base;
        let w1_bytes = OUT_CHANNELS * INPUT_CHANNELS * KERNEL_SIZE;
            ...
            while t_out < out_len {
                ...
                while k < KERNEL_SIZE {
                    ...
                    while c < INPUT_CHANNELS {
                        let x_idx = t_in * INPUT_CHANNELS + c;
                        let x = read_i32(payload_ptr + (x_idx * 4) as u64) as i64;
                        let w_idx = ((oc * INPUT_CHANNELS + c) * KERNEL_SIZE + k) as usize;
